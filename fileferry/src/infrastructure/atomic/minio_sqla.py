...
class SqlAlchemyMinioAtomicOperation(SQLAlchemyMinioAtomicContract):
    ...
    Нарушение SRP в чистом виде. Но почему так?
    Я долго думал как сделать БД и Хранилище атомарными, чтобы их состояния были идентичны всегда.
    Отсюда и рожден этот класс, он страшный, он инженерно-ужасный. Но он делает то, для чего он нужен
    и избавляет остальные слои от ответственности за сессии, состояния.
    """
